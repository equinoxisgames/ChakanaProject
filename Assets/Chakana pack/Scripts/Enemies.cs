...
public class Enemies : CharactersBehaviour
{
    ...
    void Update()
    {
        //Diferenciar el comportamiento según el tipo de Enemigo
        switch (tipoEnemigo)
        {
            ...
            case TipoEnemigo.Boraro:
                MoverPerseguirJugador();
                //Al inicio el personaje se encuentra estático
                if (RangoVision())
                {
                    MoverPerseguirJugador();
                    if (RangoCercaJugador())
    ...
    //El ataque dependerá del tipo de Enemigo
